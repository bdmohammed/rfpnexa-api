...
import { userPermissionModule, UserPermissions } from '@/constants/permissions';
...
export default class AddedUserModuleAndPermissions1720800006 implements SeedInterface {
  ...
  public async up(dataSource: DataSource, systemUser?: User): Promise<void> {
    ...
    let module = await moduleRepo.findOne({
      where: { key: userPermissionModule.key },
    });
    ...
    const newPermissions = userPermissions.filter((p) => !existingKeys.has(p.key));
