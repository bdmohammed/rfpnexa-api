 ...
 */
export async function getCountriesHierarchy(): Promise<CountryHierarchyDto[]> {
...
export async function createChangeRequest(
  ...
  return AppDataSource.transaction(async (manager) => {
    ...
    const seqRes = await manager.query(
      "SELECT nextval(COALESCE(pg_get_serial_sequence('country_change_requests', 'request_sequence'), 'country_change_requests_request_sequence_seq')) AS nextval",
    );
...
export async function listDistinctCountries(): Promise<ListDistinctCountriesResponse[]> {
