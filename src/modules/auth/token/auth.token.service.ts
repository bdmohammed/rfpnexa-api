...
/**
 * here if replay-detection is required in the future.
 */
export async function refreshSession(
  reqToken: string | undefined,
  res: Response,
  ...
  // Decode the access token from the refresh cookie to identify the user.
  // The refresh cookie carries the raw opaque token; the access token cookie
  // is separately managed. We re-read the access token from its own cookie
  // to extract the user identity needed to mint a new pair.
  const accessTokenRaw = res.req.cookies?.[process.env['JWT_COOKIE_NAME'] ?? 'rfpnexa_token'] as
    string | undefined;
  if (!accessTokenRaw) {
    throw new AppError(
      AppErrorMessage.INVALID_REFRESH_TOKEN,
  ...
  try {
    decoded = jwt.verify(accessTokenRaw, env.JWT_SECRET, {
      issuer: env.JWT_ISSUER,
      audience: env.JWT_AUDIENCE,
      ignoreExpiration: true, // allow expired access tokens during refresh
  ...
  // Build a minimal User-like object from the token claims
  const userLike = {
    id: decoded.userId,
    email: decoded.email,
    accountType: decoded.accountType,
  } as User;
  await generateAndSetTokens(res, userLike, connectionContext);
}
