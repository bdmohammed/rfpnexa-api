...
export async function listAllCategories(query: Partial<CategoryQueryDto> = {}) {
...
export async function createCategory(dto: CreateCategoryDto) {
  ...
  const exists = await categoryRepo.existsBy({
    slug,
  });
  if (exists) {
    throw new AppError(
...
export async function updateCategory(id: string, dto: CreateCategoryDto) {
