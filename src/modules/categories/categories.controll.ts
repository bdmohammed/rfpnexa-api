...
import {
  type CreateCategoryContract,
  type ListCategoriesContract,
  type ListDistinctCategoriesContract,
  type UpdateCategoryContract,
...
export const listDistinctCategories = contractAsyncHandler<ListDistinctCategoriesContract>(
  async (_req, res) => {
    const categories = await service.listDistinctCategories();
    // const stats = await service.getCategoryStats();
    return sendOk(res, categories);
  },
);

export const listCategories = contractAsyncHandler<ListCategoriesContract>(async (req, res) => {
  const { query } = req;
  const { categories, total } = await service.listAllCategories(query);
  // const stats = await service.getCategoryStats();
  return sendOk(res, categories, 'OK', paginationMeta(total, query.page, query.limit));
});
...
export const createCategory = contractAsyncHandler<CreateCategoryContract>(async (req, res) => {
  const dto = req.body;
  const category = await service.createCategory(dto);
  return sendCreated(res, category, 'Category created');
});

export const updateCategory = contractAsyncHandler<UpdateCategoryContract>(async (req, res) => {
  const dto = req.body;
  const category = await service.updateCategory(req.params.id, dto);
