...
import type { CreateTenderDto, UpdateTenderDto } from './tenders.dto';
...
import { paginationMeta, sendOk } from '@/core/response';
...
export const getTenders = asyncHandler(async (req, res) => {
  ...
  // const maxPriceCents = typeof req.query.maxPriceCents === 'string' ? req.query.maxPriceCents : undefined;
  const { tenders, total } = await service.listTenders(
    page,
    limit,
    search,
    countryId,
  ...
  return sendOk(res, tenders, '', paginationMeta(total, page, limit));
});

export const getTenderById = asyncHandler(async (req, res) => {
  const tender = await service.getTenderById(req.params.id as string);

  return sendOk(res, tender);
});

function getTenderDocuments(req: Request): Express.Multer.File[] {
...
export const createTender = asyncHandler(async (req, res) => {
  ...
  const files = getTenderDocuments(req);

  const tender = await service.createTender(req.body as CreateTenderDto, files, req.user.userId);
...
export const updateTender = asyncHandler(async (req, res) => {
  ...
  const tender = await service.updateTender(
    req.params.id as string,
    req.body as UpdateTenderDto,
    files,
  );
